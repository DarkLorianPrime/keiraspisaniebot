...
from parser.schedulechanges import send_changes
from utils.decorators import json_command_handler, command_handler
from utils.functional import is_member, send_text
from utils.schedulehelper import get_timetable, week_company, get_week
...
def today(vk_object: EventInformation) -> None:
    info = get_timetable(send_id=vk_object.send_id, chat_id=vk_object.chat_id, notsunday=True)
...
def send_tommorow_or_updated(vk_object: EventInformation, number_company: int) -> None:
    ...
    info = get_timetable(send_id=vk_object.send_id, chat_id=vk_object.chat_id)
    if info["to_days"]["day_int"] == 6:
        send_text(error="Завтра воскресение 🤔", send_id=vk_object.send_id)
    ...
    if number_company != 1:
        parsed_info = send_changes(group=group, chat_id=vk_object.chat_id, dont_send=True)
        updates = [list(i.items()) for i in parsed_info[0]]

    if number_company == 0:
        if str(datetime.datetime.now().day) in parsed_info[1]:
            send_text(error=get_text("not_updated"), send_id=vk_object.send_id)

    if number_company == 2:
        if str(datetime.datetime.now().day) not in parsed_info[1]:
            send_text(error=get_text("already_updated"), send_id=vk_object.send_id)
...
def changes_lesson(vk_object: EventInformation) -> None:
    is_member(vk_object.from_id, vk_object.send_id)
    if not query_exists(Group.chat_id, str(vk_object.chat_id)):
        send_text(error=get_text('not_group'), send_id=vk_object.send_id)
    group = Group.where(chat_id=str(vk_object.chat_id)).first().group
    send_changes(group=group, chat_id=vk_object.chat_id)
